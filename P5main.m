...
groupd = ones(N,10);
for J = 1 : 10000
[gamete_a,gamete_d] = mate(groupa,groupd);
for i = 1:2:N    
 adaption((i+1)/2) = adapt([gamete_a(i,:);gamete_a(i+1,:)],[gamete_d(i,:);gamete_d(i+1,:)]);
end
...
if H2L(1)>best
    best = H2L(1);
    c = c+1;
 bestGene_a(1,:) =  gamete_a(2*rank(1)-1,:);
 bestGene_a(2,:) =  gamete_a(2*rank(1),:);
...
for i2 = 1:N/2
    for t = b:a(i2)
        groupa(2*t,:) = gamete_a(2*rank(i2),:);
        groupa(2*t-1,:) = gamete_a(2*rank(i2)-1,:);
